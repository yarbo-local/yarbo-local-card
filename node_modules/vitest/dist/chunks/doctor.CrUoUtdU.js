...
import { t as toArray, r as resolve } from './pathe.M-eThtNZ.Clq7AXhv.js';
...
function resolveDoctorCandidates(projects, options = {}) {
	...
	const runsDom = testProjects.some((project) => DOM_ENVIRONMENTS.has(project.environment));
	...
	if (runsDom && !pools.has("vmThreads")) candidates.push({
		id: "vmThreads",
	...
	if (runsDom && !pools.has("vmForks")) candidates.push({
		id: "vmForks",
		title: `pool: 'vmForks'`,
...
async function doctor(cliFilters, options) {
	const { prepareVitest } = await import('./cli-api.LUtK11-x.js').then(function (n) { return n.a; });
	log();
