import { n as normalize, t as toArray } from './pathe.M-eThtNZ.Clq7AXhv.js';
...
async function start(cliFilters, options) {
	try {
		const { startVitest } = await import('./cli-api.LUtK11-x.js').then(function (n) { return n.a; });
		const ctx = await startVitest(cliFilters.map(normalize), normalizeCliOptions(cliFilters, options));
		if (!ctx.shouldKeepServer()) await ctx.exit();
	} catch (e) {
		const { errorBanner } = await import('./utils.CtqmNjXD.js').then(function (n) { return n.u; });
...
async function doctorCommand(cliFilters, options) {
	try {
		const { doctor } = await import('./doctor.CrUoUtdU.js');
...
async function init(project) {
	...
	const { create } = await import('./creator.BHJ7K266.js');
	await create();
}
async function collect(cliFilters, options) {
	try {
		const { prepareVitest, processCollected, outputFileList } = await import('./cli-api.LUtK11-x.js').then(function (n) { return n.a; });
