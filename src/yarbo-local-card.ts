...
export class YarboLocalCard extends LitElement {
  ...
  getCardSize(): number {
    return Math.ceil((this._config?.height ?? DEFAULT_HEIGHT) / 50) + 1;
  }

  getGridOptions() {
    return { columns: 12, rows: Math.ceil((this._config?.height ?? DEFAULT_HEIGHT) / 56) + 2, min_columns: 6, min_rows: 5 };
  }
...
if (!customElements.get("yarbo-local-card")) {
  customElements.define("yarbo-local-card", YarboLocalCard);
}
...
console.info(`%c YARBO-LOCAL-CARD %c ${VERSION} `, "color:#fff;background:#2f8f86;border-radius:3px 0 0 3px", "color:#2f8f86;background:#e8f1f0;border-radius:0 3px 3px 0");
