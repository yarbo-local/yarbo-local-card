...
g.elementStyles = [], g.shadowRootOptions = { mode: "open" }, g[m("elementProperties")] = /* @__PURE__ */ new Map(), g[m("finalized")] = /* @__PURE__ */ new Map(), ie?.({ ReactiveElement: g }), (f.reactiveElementVersions ??= []).push("2.1.2");
//#endregion
//#region node_modules/lit-html/lit-html.js
var _ = globalThis, se = (e) => e, v = _.trustedTypes, ce = v ? v.createPolicy("lit-html", { createHTML: (e) => e }) : void 0, le = "$lit$", y = `lit$${Math.random().toFixed(9).slice(2)}$`, b = "?" + y, ue = `<${b}>`, x = document, S = () => x.createComment(""), C = (e) => e === null || typeof e != "object" && typeof e != "function", w = Array.isArray, de = (e) => w(e) || typeof e?.[Symbol.iterator] == "function", T = "[ 	\n\f\r]", E = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, D = /-->/g, O = />/g, k = RegExp(`>|${T}(?:([^\\s"'>=/]+)(${T}*=${T}*(?:[^ \t\n\f\r"'\`<>=]|("|')|))|$)`, "g"), A = /'/g, j = /"/g, fe = /^(?:script|style|textarea|title)$/i, pe = (e) => (t, ...n) => ({
	_$litType$: e,
	strings: t,
	values: n
}), M = pe(1), N = pe(2), P = Symbol.for("lit-noChange"), F = Symbol.for("lit-nothing"), me = /* @__PURE__ */ new WeakMap(), I = x.createTreeWalker(x, 129);
function he(e, t) {
...
var ge = (e, t) => {
	...
	for (let t = 0; t < n; t++) {
		...
		for (; u < n.length && (o.lastIndex = u, c = o.exec(n), c !== null);) u = o.lastIndex, o === E ? c[1] === "!--" ? o = D : c[1] === void 0 ? c[2] === void 0 ? c[3] !== void 0 && (o = k) : (fe.test(c[2]) && (i = RegExp("</" + c[2], "g")), o = k) : o = O : o === k ? c[0] === ">" ? (o = i ?? E, l = -1) : c[1] === void 0 ? l = -2 : (l = o.lastIndex - c[2].length, s = c[1], o = c[3] === void 0 ? k : c[3] === "\"" ? j : A) : o === j || o === A ? o = k : o === D || o === O ? o = E : (o = k, i = void 0);
...
Se?.(L, z), (_.litHtmlVersions ??= []).push("3.3.3");
var Ce = (e, t, n) => {
...
function Fe(e, t = 0, n = []) {
	if (t > 4 || !e || typeof e != "object" || n.length > 5e3) return n;
	let r = U(e);
	if (r) return n.push(r), n;
	for (let r of Object.values(e)) Fe(r, t + 1, n);
	return n;
}
//#endregion
...
function Ze(e) {
	return Math.min(Ye, Math.max(2, e));
}
function J(e, t, n) {
	let r = Ze(e.width * t), i = r / e.width;
	return {
		cx: n[0] + (e.cx - n[0]) * i,
		cy: n[1] + (e.cy - n[1]) * i,
		width: r
...
var tt = "0.1.0", Z = 440, nt = .55, rt = 6, it = 14, at = {
	sleeping: "Sleeping",
	idle: "Idle",
	...
	charging: "Charging",
	error: "Error"
}, Q = (e) => N`<svg viewBox="0 0 24 24" aria-hidden="true"><path d=${e}></path></svg>`, $ = (e) => e.map(([e, t]) => `${-e},${-t}`).join(" "), ot = class extends H {
	static styles = Je;
	static properties = {
		hass: { attribute: !1 },
		_config: { state: !0 },
	...
	_onEvent(e) {
		...
		} else e.type === "feedback" && (this._feedback = {
			...this._feedback,
			[e.leaf]: e.data
		});
	}
	...
	_onWheel(e) {
		...
		this._view = J(this._view, Math.exp(e.deltaY * .0015), r);
	}
	_onPointerDown(e) {
	...
	_onPointerMove(e) {
		...
		if (this._pointers.set(e.pointerId, {
			...
			this._view = J(t.view, t.pinch / Math.max(1, r), i);
			return;
		}
	...
	_tap(e) {
		...
		let t = [-e[0], -e[1]], n = this._view ? X(this._view, this._size) : .05, r = this._map?.zones ?? [], i = r.find((e) => !e.closed && We(t, e.points) < 10 * n), a = r.filter((e) => e.closed && Ue(t, e.points)), o = i ?? a.sort((e, t) => (e.area_m2 ?? 0) - (t.area_m2 ?? 0))[0];
	...
	_pick(e) {
		...
		switch (t.step) {
			...
			case "photo2":
				...
				break;
			case "map2": {
				let n = this._snap(e), r = t.u1 && t.u2 && t.w1 ? ze(t.u1, t.u2, t.w1, n) : null;
				this._align = r ? {
					...t,
					w2: n,
					result: r,
	...
	_renderHeader() {
      ...
      <div class="chips">
        ${e && !e.connected ? M`<span class="chip bad">${Q(je)}Offline</span>` : F}
        ${e ? M`<span class="chip ${e.activity === "error" ? "bad" : ""}"
              >${e.awake === !1 ? Q(Ne) : F}${at[e.activity] ?? e.activity}</span
            >` : F}
        ${e?.battery == null ? F : M`<span class="chip ${e.battery < 20 ? "bad" : ""}"
              >${Q(e.charging ? Ee : Te)}${e.battery}%</span
            >`}
        ${e ? M`<span class="chip ${r.cls}"
              >${Q(Me)}${r.label}${e.satellites ? M` · ${e.satellites}` : F}</span
            >` : F}
      </div>
    </div>`;
	}
	_renderSvg() {
      ...
      <g class=${c ? "dim" : ""}>
        ...
        ${this._renderFeedback()} ${this._renderTrail()} ${this._renderRobot(o)} ${this._renderLabels(o)}
      </g>
      ${this._renderPicks(o)}
    </svg>`;
	}
	_renderBackground() {
		...
		if (e?.width && e.height) {
			...
			return N`<image href=${e.url} width=${e.width} height=${e.height} transform=${He(n)}
        opacity=${r} preserveAspectRatio="none"></image>`;
		}
		let t = this._background;
		return t ? N`<image href=${t.image} width=${t.width} height=${t.height} transform=${He(t.transform)}
	...
	_renderDocks() {
		return (this._map?.docks ?? []).map((e) => {
			let t = e.straight_phi ?? 0, { dock: n, guard: r } = Le(e.point, t);
			return N`<polygon class="guard" points=${$(r)}></polygon>
        <polygon class="dock" points=${$(n)}></polygon>`;
		});
	}
	_renderFeedback() {
		let e = [], t = Pe(this._feedback.plan_feedback);
		for (let n of t.remaining) e.push(N`<polyline class="plan-remaining" points=${$(n)}></polyline>`);
		for (let n of t.visited) e.push(N`<polyline class="plan-visited" points=${$(n)}></polyline>`);
		let n = G(this._feedback.recharge_feedback);
		n.length >= 2 && e.push(N`<polyline class="route" points=${$(n)}></polyline>`);
		for (let [t, n] of Fe(this._feedback.cloud_points_feedback)) e.push(N`<rect class="obstacle" x=${-t - .06} y=${-n - .06} width="0.12" height="0.12"></rect>`);
		return e;
	}
	_renderTrail() {
		return this._config?.trail === !1 ? F : Ke(this._trail).map((e) => {
			let t = `trail ${e.reverse ? "reverse" : ""} ${e.working ? "" : "thin"}`;
			return e.working ? N`<polyline class=${t} stroke-width=${nt} points=${$(e.points)}></polyline>` : N`<polyline class=${t} points=${$(e.points)}></polyline>`;
		});
	}
	_renderRobot(e) {
		let t = this._live;
		if (!t || t.x === null || t.y === null || t.phi === null) return F;
		let n = Re(t.x, t.y, t.phi), [r, i] = K(t.x, t.y);
		return N`${1.3 / e < 22 ? N`<circle class="robot-halo" cx=${r} cy=${i} r=${11 * e}></circle>` : F}
	...
	_renderPicks(e) {
		...
		let n = 6 * e, r = t.step === "preview" && t.result ? t.result : t.placement, i = [t.u1, t.u2].filter((e) => !!e).map((e) => Be(r, e)), a = [t.w1, t.w2].filter((e) => !!e);
